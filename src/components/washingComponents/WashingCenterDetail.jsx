...
const WashingCenterDetail = ({ center, onClose, onBookNow }) => {
  ...
  useEffect(() => {
    const loadGarageDetails = async () => {
      ...
      try {
        setLoading(true);
        const data = await fetchGarageById(center.id);
        if (data) {
  ...
  const handleDirectionClick = () => {
    ...
    const propLoc = center?.location;
    const lat = apiLoc?.latitude || apiLoc?.lat || garageData?.latitude || propLoc?.latitude || propLoc?.lat || center?.latitude;
    const lng = apiLoc?.longitude || apiLoc?.lng || garageData?.longitude || propLoc?.longitude || propLoc?.lng || center?.longitude;
    if (lat && lng) {
    ...
    const address = garageData?.address || center?.address || '';
    if (address) {
  ...
  const getSanitizedPhone = () => {
    const rawPhone = garageData?.phone || center?.phone;
    if (!rawPhone) return '';
  ...
  // Use only API data - no fallback to passed center data
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4">
                <img
                  ...
                  alt={displayGarage?.name || 'Washing Center'}
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {(displayGarage?.is_verified || center?.is_verified) && (
            ...
            <div ref={aboutRef} className="space-y-4 sm:space-y-6 md:space-y-8">
              ...
              <div className={`rounded-xl p-4 sm:p-6 md:p-8 border ${
                ...
                <p className={`text-sm sm:text-base md:text-base leading-relaxed ${theme === 'light' ? 'text-gray-700' : 'text-gray-300'}`}>
                  {displayGarage?.description || "Professional automotive washing and detailing center providing comprehensive cleaning and maintenance services for all types of vehicles. Our experienced team ensures quality service and customer satisfaction."}
                </p>
              </div>
              ...
              {displayGarage?.services?.service && displayGarage.services.service.length > 0 ? (
                  ...
                  <div key={index} className="mb-6 sm:mb-8">
                    <div className="flex items-center mb-4 sm:mb-5 pb-2 sm:pb-3 border-b-2 border-red-500/20">
                      <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 bg-gradient-to-br from-red-500 via-red-600 to-red-700 rounded-lg sm:rounded-xl flex items-center justify-center mr-3 sm:mr-4 shadow-lg shadow-red-500/30">
                        <FontAwesomeIcon icon={faSprayCan} className="text-white text-base sm:text-xl md:text-2xl" />
                      </div>
                      <div>
                ...
                <div className={`rounded-xl sm:rounded-2xl p-8 sm:p-12 md:p-16 text-center border-2 ${theme === 'light' ? 'bg-gradient-to-br from-gray-50 to-white border-gray-200' : 'bg-gradient-to-br from-gray-800 to-gray-900 border-gray-700'}`}>
                  <div className={`w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 mx-auto mb-4 sm:mb-6 rounded-full flex items-center justify-center ${
                    ...
                    <FontAwesomeIcon icon={faSprayCan} className={`text-3xl sm:text-4xl md:text-5xl ${theme === 'light' ? 'text-gray-400' : 'text-gray-500'}`} />
                  </div>
