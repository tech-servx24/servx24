...
const GarageBookingFlow = ({ garageId: propGarageId, vehicleType: propVehicleType, onComplete, onCancel }) => {
  ...
  useEffect(() => {
    try {
      console.log("GarageBookingFlow mounted with garageId:", garageId);
      ...
      fetchGarageInfo();
      console.log("GarageBookingFlow ready with garageId:", garageId);
    } catch (error) {
      console.error("Error in GarageBookingFlow useEffect:", error);
      if (onCancel) {
        onCancel();
      } else {
        navigate("/");
        ...
        <div className={`text-center ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>
          ...
          <button
            onClick={() => {
              if (onCancel) {
                onCancel();
              } else if (returnTo === 'garage-list' && vehicleType) {
                navigate(`/?vehicleType=${vehicleType}`);
              } else {
                navigate("/");
              }
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${theme === 'light' ? 'bg-white' : 'bg-black'}`}>
        <div className="text-center">
          ...
          <p className={`${theme === 'light' ? 'text-gray-700' : 'text-gray-400'}`}>Loading garage information...</p>
        </div>
      </div>
    );
  }
