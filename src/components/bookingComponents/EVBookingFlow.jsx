...
const EVBookingFlow = ({ garageId: propGarageId, vehicleType: propVehicleType, onComplete, onCancel }) => {
  ...
  const steps = ["Select Vehicle", "EV Service", "Slot & Address", "Summary"];
  
  // Check authentication and fetch garage info
  useEffect(() => {
    try {
      console.log("EVBookingFlow mounted with garageId:", garageId);
      ...
      const fetchGarageInfo = async () => {
        try {
          ...
          if (garageData) {
            ...
            console.log("EV Garage info loaded:", garageData);
          } else {
            console.error("Failed to load EV garage information");
            if (onCancel) {
              onCancel();
            } else {
              navigate("/");
          ...
          console.error("Error fetching EV garage info:", error);
          if (onCancel) {
            onCancel();
          } else {
            navigate("/");
      ...
      console.error("Error in EVBookingFlow useEffect:", error);
      if (onCancel) {
        onCancel();
      } else {
        navigate("/");
  ...
  const handleNext = () => {
    ...
    if (activeStep === 1 && !selectedService) {
      setErrors({ service: "Please select at least one EV service to continue" });
      return;
    }
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${theme === 'light' ? 'bg-white' : 'bg-black'}`}>
        <div className={`text-center ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>
          ...
          <p className={`mb-6 ${theme === 'light' ? 'text-gray-600' : 'text-gray-400'}`}>Please select an EV service garage first to start booking.</p>
          <button
            onClick={() => {
              if (onCancel) {
                onCancel();
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${theme === 'light' ? 'bg-white' : 'bg-black'}`}>
        <div className="text-center">
          ...
          <p className={`${theme === 'light' ? 'text-gray-700' : 'text-gray-400'}`}>Loading EV service garage information...</p>
        </div>
      </div>
    );
  }
